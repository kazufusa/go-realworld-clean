...
import (
	...
	"github.com/kazufusa/go-realworld-clean/testData"
	"github.com/stretchr/testify/assert"
)

var (
	art1 = domain.Article{Slug: "1"}
	art2 = domain.Article{Slug: "2"}
	art3 = domain.Article{Slug: "3"}
	art4 = domain.Article{Slug: "4"}
)
...
func TestAriticleIsFavoritedBy(t *testing.T) {
	...
	t.Run("without userName", func(t *testing.T) {
		filter := domain.ArticleIsFavoritedBy("")
		assert.False(t, filter(domain.Article{FavoritedBy: []domain.User{{Name: ""}}})) // always returns false
	})
}
